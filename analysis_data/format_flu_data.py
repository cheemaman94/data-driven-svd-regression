...
def normalize_location(x) -> str:
    """
    Normalize location codes so admissions and locations files match.
    Examples:
      1 or "1" or "1.0" -> "01"
      "US" stays "US"
    """
...
def _mmwr_info(dt: pd.Timestamp) -> tuple[int, int]:
    """Return (epiyear, epiweek) for a given date."""
...
def main() -> None:
    ...
    # Drop rows with missing critical fields (cannot be merged / interpreted)
    ...
    # Start date of MMWR week 40 for each season_start_year
    season_start_date = {
        y: Week(y, 40).startdate() for y in adm["season_start_year"].unique()
    }

    # season_week = whole weeks since season start (week 40)
