...
fetch_flu <- function(){
  ...
  health_data = RSocrata::read.socrata(url = "https://data.cdc.gov/resource/ua7e-t2fy.json?$limit=999999") #%>%
  
    #dplyr::filter(weekendingdate >= as.Date("2021-09-01"))
...
write.csv(target_data, file = "./analysis_data/target-hospital-admissions.csv", row.names = FALSE)
